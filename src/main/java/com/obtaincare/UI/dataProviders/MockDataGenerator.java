...
public class MockDataGenerator {
    ...
    public int generateOfficeCodes() throws SQLException {
        ...
        return officeCodes.get(faker.number().numberBetween(0,officeCodes.size()));
    }

    public int generateReportsToCodes() throws SQLException {
        List<Integer> reportsTo = EmployeeBean.getReportTos();
        return reportsTo.get(faker.number().numberBetween(0,reportsTo.size()));
    }

    public String generateJobTitles() throws SQLException {
        List<String> jobTitles = EmployeeBean.getJobTitles();
        return jobTitles.get(faker.number().numberBetween(0, jobTitles.size()));
    }
}
